...
class Trade:
    ...
    def close_trade(self, row, result, trigger_price, acumulated_loss):
        ...
        if result < 0.0:
            acumulated_loss += abs(result)
        elif result > acumulated_loss:
            print("zerou agora")
            acumulated_loss = 0.0
    ...
    def update(self, row, acumulated_loss):
        ...
        self.count += 1
        if self.SIGNAL == BUY:
            if acumulated_loss == 0.0 and row.bid_h >= self.TP:
                acumulated_loss = self.close_trade(row, self.profit_factor, row.bid_h, acumulated_loss)
            elif row.SIGNAL == SELL:
                result = (row.bid_c - self.start_price) / self.pip_value
                acumulated_loss = self.close_trade(row, result, row.bid_c, acumulated_loss)
            elif acumulated_loss > 0.0:
                result = (row.bid_h - self.start_price) / self.pip_value
                if result >= acumulated_loss:
                    print("supostamente zerou compra",result, row.bid_h, acumulated_loss)
                    acumulated_loss = self.close_trade(row, result, row.bid_h, acumulated_loss)

        if self.SIGNAL == SELL:
            if acumulated_loss == 0.0 and row.ask_l <= self.TP:
                acumulated_loss = self.close_trade(row, self.profit_factor, row.ask_l, acumulated_loss)
            elif row.SIGNAL == BUY:
                ...
                if result >= acumulated_loss:
                    print("supostamente zerou venda", result, row.ask_l, acumulated_loss)
                    acumulated_loss = self.close_trade(row, result, row.ask_l, acumulated_loss)
        
        return acumulated_loss
    

class EmaCrossMultiTemporal2Tester:
    ...
    def run_test(self):
        ...
        for index, row in self.df.iterrows():
            ...
            for ot in open_trades_m5:
                self.acumulated_loss = ot.update(row, self.acumulated_loss)
                if ot.running == False:
